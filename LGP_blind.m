function [kernel, interim_latent] = LGP_blind(y, lambda_grad, opts, lambda_lgp, betamax)
% 
...
% maxitr=max(ceil(log(5/min(opts.kernel_size))/log(ret)),0);
num_scales = maxitr + 1;
...
%     end
  ...
  [ks, lambda_grad, interim_latent] = LGP_main(ys, ks,...
      lambda_grad, threshold, opts, lambda_lgp, betamax);
%   lgplambda = lgplambda * 0.9;
    lambda_lgp = lambda_lgp / 1.1;
   %% center the kernel
%    temp = get_maxgra(interim_latent,floor(size(ks,1)*1.5));
%    name = strcat('lmg',num2str(s),'.png');
%    imwrite(temp/2,name);
...
%   tmpmat = load('3-带大像差成像和PSF数据.mat');
%   tmppsf = tmpmat.psf./sum(tmpmat.psf(:));
%   k = imresize(tmppsf,[minsize,minsize],'bicubic');
  k = k./sum(k(:));
end
