function [k, lambda_grad, S] = LGP_main(blur_B, k, ...
...
for iter = 1:opts.xk_iter
  ...
  S = LGP_Restoration(blur_B_w, k, lambda_grad, 2.0,lambda_lgp, betamax);   
  S = S(1:H,1:W,:);
...
%   fprintf('Cancelled\n');
  %
  if size(k,1)~=opts.kernel_size || opts.bw == 1
      CC = bwconncomp(k,8);
      for ii=1:CC.NumObjects
          currsum=sum(k(CC.PixelIdxList{ii}));
          if currsum<opts.bwnum % 0.1
